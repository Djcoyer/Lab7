...
public class Lab7Part1c {
    ...
    public static void main(String[] args) {
        ...
        double[][] shoppingNumbers = new double[max][max];
        String[][] shoppingNames = new String[max][max];
        Scanner inventory;
        Scanner shoppingList;
        int idIndex = 1;
        int aisleIndex = 2;
        int costIndex = 3;
        int quantityIndex = 4;
        ...
        try{
            File shoppingFile = new File("Shopping.txt");
            shoppingList = new Scanner(shoppingFile);
        }catch(FileNotFoundException fnf)
        {
            shoppingList = new Scanner(System.in);
        }
...
*/
       ...
       System.out.printf("$%.2f", inventoryValue(numbers, costIndex, quantityIndex));
       ...
       for(int z = 0; z < max; z++)
       {
           ...
           valid=true;
               if(shoppingList.hasNextInt() && valid) 
           {
               shoppingNumbers[index][quantityIndex] = shoppingList.nextInt();
           }
           else valid=false;
           if(shoppingList.hasNext() && valid)
           {
            listLine = shoppingList.nextLine();
            list=new Scanner(listLine);
           }
           else valid=false; 
           if(list.hasNext() && valid)
            {
                shoppingNames[index][categoryIndex]=list.next();
            }
           else valid=false;
           
           if(list.hasNext())
               shoppingNames[index][subCategoryIndex]=list.next();
           else shoppingNames[index][subCategoryIndex]=null;
           
           if(list.hasNext())
               shoppingNames[index][nameIndex]=list.nextLine();
           else shoppingNames[index][nameIndex]=null;
           if(!valid)
               index--;
       ...
       for(int i = 0; i < shoppingNames.length; i++)
       {
           if(shoppingNumbers[i][costIndex] > 0)
           {
           ...
           {   
                ...
                {
                    if(shoppingNames[i][index] != null)
                    System.out.print(shoppingNames[i][index] + " ");
                }
           }
           }
    ...
    public static double inventoryValue(double[][] numbers, int costIndex, int quantityIndex)
    {
        double cost = 0.0;
        
        for(int i = 0; i < numbers.length; i++)
        {
            cost+= numbers[i][costIndex]*numbers[i][quantityIndex];
        }
        return cost;
    }
